...
//https://api.twitter.com/1.1/statuses/user_timeline.json?screen_name=twitterapi&count=2
		...
		startTweetStream: function(collectionName, criteria, cb){
			...
			var newStream = {
				streamID: getNewDate,
				socket:socketObject,
				streamObject: grabConnection.twit_object.stream('statuses/filter', { track:filteredQuery })
			}

			console.log('ID',newStream.streamID)

			var logTheseEvents = ['warning','connect','disconnect','limit'];
			logTheseEvents.forEach(function(v){
				console.log('Logging',v)
				newStream.streamObject.on(v,console.log);
			});
			...
			var eatTweets = function(tweet){
				...
				thisConnection.socket.emit('streamMessage',tweet)
			};

			newStream.streamObject.on('tweet',eatTweets);

			grabConnection.tweetStreams[getNewDate] = newStream;

			return cb(null,makeDate.getTime());
